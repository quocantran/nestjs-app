...
export class MailService {
  ...
  async sendMailToSubscribers() {
    try {
      const limit = 50;
      const totalSubscribers = await this.subscriberService.count();
      const totalPages = Math.ceil(totalSubscribers / limit);

      let page = 1;

      while (page <= totalPages) {
        const subscribers = await this.subscriberService.getAll(page, limit);
        ...
        const sendMailPromises = subscribers.map(async (subscriber) => {
          ...
          const jobsFormatted = jobs.map((job) => {
            return {
              ...job,
              jobUrl: `${this.configService.get<string>('URL_FRONTEND')}/jobs/${job._id}`,
            };
          });
