...
const Subscriber = (props: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsFetching(true);
      const skillRes = await fetchSkills({ pageSize: 100, current: 1 });
      if (skillRes && skillRes.data) {
        const skillData = skillRes.data.result as ISkill[];
      ...
      const res = await getSubscriberByEmail();
      if (res && res.data) {
        const skillList = res.data.skills.map((skill: ISkill) => skill._id);
        console.log(skillList);

        setSelectedSkills(skillList as string[]);
      }
      setIsFetching(false);
    };
    fetchData();
  ...
  const handleClick = async () => {
    const data: ISubscribers = {
      email: email,
      skills: selectedSkills as unknown as ISkill[],
    };
    ...
    if (!res.ok) {
      notification.error({
        message: "Cập nhật kỹ năng thất bại",
    ...
    <>
      ...
      <div style={{ maxWidth: "600px", margin: "0 auto" }}>
        <div style={{ marginBottom: "15px" }}>
          <Input readOnly value={email} placeholder="Nhập vào email của bạn" />
        </div>
